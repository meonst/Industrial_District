...
global chatHistory, teamBlue, teamRed, players
...
def open_replay(replay_file):
    ...
    gameDetails = dict()
    gameDetails['mapName'] = details['m_title'].decode()
    gameDetails['gameVersion'] = "{}.{}.{}.{}".format(header['m_version']['m_major'], header['m_version']['m_minor'], header['m_version']['m_revision'], header['m_version']['m_baseBuild'])
    gameDetails['gameTime'] = datetime.fromtimestamp((details['m_timeUTC'] + details['m_timeLocalOffset']) // 10000000 - 11644506000)
    gameDetails['gameLength'] = '{0:02d}:{1:02d}'.format(int(header['m_elapsedGameLoops'] / 16) // 60, int((header['m_elapsedGameLoops'] / 16) % 60))
    gameDetails['gameMode'] = "Unknown"
    if initdata['m_syncLobbyState']['m_gameDescription']['m_gameOptions']['m_ammId'] in gameModeDict:
        gameDetails['gameMode'] = gameModeDict[initdata['m_syncLobbyState']['m_gameDescription']['m_gameOptions']['m_ammId']]
        ...
        for event in protocol.decode_replay_tracker_events(contents):
            ...
            if 'm_eventName' in event:
                ...
                if event['m_eventName'].decode() == 'EndOfGameTimeSpentDead':
                    ...
                    players[player_number]['TimeSpentDead'] = time_spent_dead
                ...
                if event['m_eventName'].decode() == 'EndOfGameTalentChoices':
                    ...
                    for i in event['m_stringData']:
                        if i['m_key'].decode() == 'Hero':
                            ...
                            players[player_number]['heroName'] = thisHeroName
                            players[player_number]['heroLink'] = players[player_number]['Hero']
                            players[player_number]['partyPanelPortrait'] = herodata[players[player_number]['heroLink']]['portraits']['partyPanel']
                            players[player_number]['minimap'] = herodata[players[player_number]['heroLink']]['portraits']['minimap']
                            
                #Talents
                        elif i['m_key'].decode() != 'Win/Loss' and i['m_key'] != 'Map':
                            for which_talent in herodata[players[player_number]['Hero']]["talents"]["level{}".format(talent_tier[j])]:
                                
                                if which_talent["nameId"] == i['m_value'].decode():
                                    players[player_number]['talentIcon'][j - 1] = which_talent['icon']
                ...
                if event['m_eventName'].decode() == 'PlayerDeath':
                    ...
                    for i in range(1, len(event['m_intData'])):
                        ...
                        players[player_number]['PlayerKill'].append((time, dead_player_number))
                ...
                if event['m_eventName'].decode() == 'RegenGlobePickedUp':
                    time = looptime(event['_gameloop'])
                    player_number = event['m_intData'][0]['m_value'] - 1
                    players[player_number]['RegenGlobesTime'].append(time)
    ...
    for i in details['m_playerList']:
        ...
        players[player_number]['playerName'] = i['m_name'].decode()
        player_number += 1
    ...
    for i in excludeFromStats:
        stats.pop(i, None)
    return [chatHistory, players, stats, statistics, gameDetails]
...
def replay_page():
    if request.method == 'POST':
        ...
        for i in returnData[0]:
            chatlog += "{}({}:{}){}: {}".format("\n", format(int(i[0]//60), '02d'), format( int(i[0]%60), '02d'), players[int(i[1])]['playerName'], i[2].decode()) 
        ...
        return replay_template.render(cssURL=cssURL, jsURL=jsURL, chatlog=chatlog, players=returnData[1], stats=returnData[2], statistics=returnData[3], gameDetails=returnData[4], chartTitle=chartTitle, chartLink=chartLink, chartLinkID=chartLinkID)
